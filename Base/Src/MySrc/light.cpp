...
{
	// ライト番号変更
	if (CInputManager::RefInstance().GetKeyboard()->GetTrigger(DIK_LSHIFT))
	{
...
{
	// X方向変更
	if (CInputManager::RefInstance().GetKeyboard()->GetPress(DIK_5))
	{
		m_light[m_nID].Direction.x += 0.1f;
	}
	else if (CInputManager::RefInstance().GetKeyboard()->GetPress(DIK_6))
	{
		m_light[m_nID].Direction.x -= 0.1f;
	}

	// Y方向変更
	if (CInputManager::RefInstance().GetKeyboard()->GetPress(DIK_7))
	{
		m_light[m_nID].Direction.y += 0.1f;
	}
	else if (CInputManager::RefInstance().GetKeyboard()->GetPress(DIK_8))
	{
		m_light[m_nID].Direction.y -= 0.1f;
	}

	// Z方向変更
	if (CInputManager::RefInstance().GetKeyboard()->GetPress(DIK_9))
	{
		m_light[m_nID].Direction.z += 0.1f;
	}
	else if (CInputManager::RefInstance().GetKeyboard()->GetPress(DIK_0))
	{
		m_light[m_nID].Direction.z -= 0.1f;
	}
}
...
void CLight::Reset()
{
	if (CInputManager::RefInstance().GetKeyboard()->GetTrigger(DIK_R))
	{
		// ライトの方向をリセット
